...
<section id="about" bind:this={section} class="section-wrap py-20 sm:py-28 overflow-x-hidden" class:reveal={visible}>
	<div class="max-w-6xl mx-auto px-4 sm:px-6">
		<div class="section-label font-mono text-xs mb-3">01 — About</div>
		<h2 class="section-title text-3xl sm:text-4xl md:text-5xl font-black mb-10 sm:mb-16 leading-tight">
		...
		<div class="grid lg:grid-cols-5 gap-10 sm:gap-12">
			<!-- Left: bio + stats -->
			<div class="lg:col-span-3 space-y-6 sm:space-y-8 stagger-1">
				<p class="about-text text-base sm:text-lg leading-relaxed">
				...
				<p class="about-text text-sm sm:text-base leading-relaxed" style="color: var(--color-text-muted)">
				...
				<div class="stats-row grid grid-cols-3 gap-3 sm:gap-4 pt-2">
			...
			<div class="lg:col-span-2 space-y-6 stagger-2">
				<div class="timeline">
					<h3 class="timeline-title text-xs sm:text-sm font-semibold mb-4 font-mono uppercase tracking-wider">Experience</h3>
					{#each experience as job, i}
						<div class="timeline-item" style="animation-delay: {0.1 + i * 0.1}s">
							<div class="timeline-dot"></div>
							<div class="timeline-content">
								<div class="flex items-start justify-between gap-2 flex-wrap">
									<span class="font-semibold text-xs sm:text-sm leading-snug">{job.role}</span>
									<span class="period-badge font-mono text-xs whitespace-nowrap">{job.period}</span>
								</div>
								<div class="text-xs mt-0.5" style="color: var(--color-text-faint)">{job.type}</div>
							</div>
						</div>
					{/each}
				</div>

				<div class="edu-block rounded-xl p-4 sm:p-5">
					<h3 class="text-xs sm:text-sm font-semibold mb-4 font-mono uppercase tracking-wider" style="color: var(--color-text-muted)">Education</h3>
					<div class="space-y-3">
						{#each education as edu}
							<div class="edu-item">
								<div class="text-xs sm:text-sm font-medium leading-tight">{edu.degree}</div>
								<div class="text-xs mt-0.5 flex items-start justify-between gap-2 flex-wrap">
									<span style="color: var(--color-text-faint)">{edu.institution}</span>
									<span class="period-badge font-mono text-xs flex-shrink-0">{edu.period}</span>
								</div>
							</div>
						{/each}
					</div>
...
<style>
	...
	.about-text { color: var(--color-text); }
